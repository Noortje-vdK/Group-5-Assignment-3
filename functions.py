...
def data_processing(filename, testing = True):
    ...
    data.dropna()
    if testing:
        return data.drop(data[(data['target_feature'] != 0) & (data['target_feature'] != 1)].index) # remove row if target feature is not binary
    else:
        return data.drop(columns=["Unique_ID"]) # drop unique ID for easier handling
...
def remove_correlated_variables(data, threshold):
    """From data that only has numerical or binary columns, remove columns with variables that are highly correlated: that have a correlation value above the threshold"""
